...
type Package struct {
	...
	Deps []string // TODO -- This can be used to list all the dependencies in a package, so that we don't filter at the command line
}

func listPackagesImported(packageName string) (packages []string, err error) {
	cmd := exec.Command(
		"go", "list",
		"-json",
		packageName,
	...
	fmt.Fprintf(os.Stderr, "ImportMap: %v\n", p.ImportMap)
...
func main() {
	...
	//
	packageList, err := listPackagesImported(os.Args[1])
	if err != nil {
